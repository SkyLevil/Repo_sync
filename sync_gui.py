...
class SyncEngine:
    ...
    def sync_pairs(self, repo_root: Path, pairs: List[SyncPair], two_way: bool, delete_stale: bool) -> None:
        for pair in pairs:
            source = repo_root / pair.repo_subfolder
            target = pair.windows_folder

            if not source.exists():
                self._logger(f"[SKIP] Repo folder does not exist: {source}")
                continue
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Repo to Windows Folder Sync")
        self.resize(920, 620)

        self.repo_root_edit = QLineEdit()
        self.repo_root_edit.setPlaceholderText("Select your repository root folder")

        browse_repo_btn = QPushButton("Browse")
        browse_repo_btn.clicked.connect(self._pick_repo_root)

        self.table = QTableWidget(0, 2)
        self.table.setHorizontalHeaderLabels(["Repo subfolder", "Windows target folder"])
        ...
        remove_row_btn = QPushButton("Remove selected mapping")
        ...
        top_grid.addWidget(QLabel("Repo root"), 0, 0)
        ...
        # Prepopulate with two mappings, matching the common scenario.
        self._add_row()
        self._add_row()
    ...
    def _pick_target_for_selected(self) -> None:
        ...
        selected = QFileDialog.getExistingDirectory(self, "Select Windows target folder")
        if selected:
            self.table.setItem(row, 1, QTableWidgetItem(selected))

    def _collect_pairs(self) -> Tuple[Path, List[SyncPair]]:
        repo_root_text = self.repo_root_edit.text().strip()
        if not repo_root_text:
            raise ValueError("Please choose a repository root folder.")

        repo_root = Path(repo_root_text)
        if not repo_root.exists():
            raise ValueError("Repository root folder does not exist.")
        ...
        for row in range(self.table.rowCount()):
            ...
            if src_path.is_absolute():
                raise ValueError(
                    f"Row {row + 1}: repo subfolder must be relative to repo root, not absolute."
                )

            pairs.append(SyncPair(repo_subfolder=src_path, windows_folder=Path(dst)))

        if not pairs:
            raise ValueError("Please define at least one folder mapping.")

        return repo_root, pairs

    def _run_sync(self) -> None:
        self.log.clear()
        try:
            repo_root, pairs = self._collect_pairs()
        ...
        except Exception as exc:  # surface any error in GUI
